...
#     caffe.log.test (columns: '#Iters Seconds TestMse')
...
grep 'Test net output #0' aux.txt | awk '{print $12}' > aux1.txt
...
`cp aux4.txt auxx4.txt`

# Generating
echo '#Iters Seconds TestMse'> $LOG.test
paste aux0.txt aux4.txt aux1.txt | column -t >> $LOG.test
rm aux.txt aux0.txt aux1.txt aux3.txt aux4.txt
